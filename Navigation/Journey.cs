...
namespace IngameScript
{
    public class Journey : ICruiseController
    {
        ...
        public void InitStep(int index)
        {
            ...
            // Per-leg profile. Set unconditionally rather than only when overridden, because
            // thrustControl is shared across legs and would otherwise keep the previous leg's
            // value. INHERIT falls back to the global config.
        ...
        //format: <speed> <stopAtWaypoint: true/false> [thrustRatio] [brakingSafetyFactor] <gps>
        //
        // The two optional numbers override MaxThrustOverrideRatio and BrakingSafetyFactor for
        // that leg alone. They are recognised positionally: any token after the first two that
        // parses as a number and is not the GPS belongs to the profile. GPS tokens always start
        // with "GPS:", so there is nothing to disambiguate and old journey lines still parse.
        public static bool TryParseWaypoint(string line, out Waypoint waypoint)
        {
            ...
            {
                double thrustRatio = INHERIT;
                double brakingSafetyFactor = INHERIT;
                ...
                for (int i = 2; i < args.Length && found < 2; i++)
                {
                    ...
                    if (!double.TryParse(args[i], out val))
                        continue;
                    if (found == 0) thrustRatio = val;
                    else brakingSafetyFactor = val;
                    found++;
                }
