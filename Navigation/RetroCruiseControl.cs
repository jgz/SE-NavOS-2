...
namespace IngameScript
{
    public class RetroCruiseControl : OrientControllerBase, ICruiseController
    {
        ...
        /// <summary>
        /// The deceleration the stopping-distance model is allowed to assume. Every "can I still
        /// stop from here" prediction uses this; the thrust actually commanded once braking uses
        /// the real <see cref="_forwardAccelPremult"/>, so a low factor starts the burn earlier
        /// rather than braking weaker. Computed, not cached, because UpdateThrustAndAccel runs in
        /// the constructor - before Journey can set the per-leg override.
        /// </summary>
        ...
        public void Run()
        {
            ...
            if (_stage == CruiseStage.Accelerate)
            {
                ...
                else
                {
                    ...
                    double d2 = 2 * BrakingAccel; // braking side is derated
